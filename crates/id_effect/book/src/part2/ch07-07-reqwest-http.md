...
The **`id_effect_platform::http::reqwest`** module integrates **`reqwest::Client`** with the effect environment: the client lives in **`R`** behind **`ReqwestClientKey`**, and HTTP work is expressed as **`Effect`** values (`send`, `text`, `bytes`, `json`, …).

## Relation to portable HTTP

| Use | Module |
|-----|--------|
| Portable **`HttpClient`** trait, swap implementations in tests, minimal request/response model | [`id_effect_platform::http`](./ch07-06-platform-services.md) (`HttpClientKey`, `execute`, …) |
